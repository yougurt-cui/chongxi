...
class DemandCrossAnalysisApiTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_requires_admin_token(self):
        response = self.client.post("/api/demand-cross-analysis/run", json={})
        self.assertEqual(response.status_code, 401)

    def test_rejects_non_boolean_dry_run(self):
        response = self.client.post(
            "/api/demand-cross-analysis/run",
            json={"dry_run": "false"},
            headers=self.headers,
        )
        self.assertEqual(response.status_code, 400)

    @patch("api.demand_cross_analysis_api.run_demand_cross_analysis")
    def test_runs_full_replace_by_default(self, run):
