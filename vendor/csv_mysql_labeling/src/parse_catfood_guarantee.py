...
def _guarantee_dedupe_keys(item: Dict[str, Any]) -> List[Tuple[str, str, Decimal, str]]:
    ...
    keys: List[Tuple[str, str, Decimal, str]] = []
    for normalized_name, operator_symbol, metric_value, metric_unit, _ in _parse_guarantee_measurements(
        metric_name=metric_name,
        raw_value=raw_value,
    ):
        keys.append((normalized_name, operator_symbol, _quantize_2(metric_value), metric_unit))
    return keys


def _merge_extracted_with_ocr_text(extracted: Dict[str, Any], ocr_text: Optional[str]) -> Dict[str, Any]:
    merged = dict(extracted or {})
    if not ocr_text:
        return merged

    fallback = _extract_guarantee_from_ocr_text(ocr_text=ocr_text)
    fallback_guarantees = fallback.get("guarantees") or []
    if not fallback_guarantees:
        return merged

    existing_guarantees: List[Dict[str, Any]] = []
    seen: set[Tuple[str, str, Decimal, str]] = set()
    for item in merged.get("guarantees") or []:
        if not isinstance(item, dict):
            continue
        existing_guarantees.append(dict(item))
        seen.update(_guarantee_dedupe_keys(item))

    for item in fallback_guarantees:
        if not isinstance(item, dict):
            continue
        item_keys = _guarantee_dedupe_keys(item)
        if not item_keys:
            continue
        if all(key in seen for key in item_keys):
            continue
        existing_guarantees.append(dict(item))
        seen.update(item_keys)

    merged["guarantees"] = existing_guarantees
    if not _clean_extracted_text(merged.get("metab_energy"), fallback=""):
        merged["metab_energy"] = fallback.get("metab_energy") or merged.get("metab_energy") or ""
    if not _normalize_basis_value(merged.get("basis")):
        merged["basis"] = fallback.get("basis") or merged.get("basis") or ""
    return merged
...
def _convert_extracted_data(
    ...
    extracted = _merge_extracted_with_ocr_text(extracted, ocr_text)
...
def ensure_product_guarantee_tables(
    ...
    failure_table: str = DEFAULT_FAILURE_TABLE,
) -> None:
    info_table = _safe_table(info_table)
    guarantee_table = _safe_table(guarantee_table)
    failure_table = _safe_table(failure_table)
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text(create_failure_sql))
        ...
        failure_indexes = _table_indexes(conn, failure_table)
        if "uq_source_id" not in failure_indexes:
            conn.execute(text(f"ALTER TABLE `{failure_table}` ADD UNIQUE KEY uq_source_id (source_id)"))
        if "idx_parsed_row_id" not in failure_indexes:
            conn.execute(text(f"ALTER TABLE `{failure_table}` ADD KEY idx_parsed_row_id (parsed_row_id)"))
        if "idx_image_name" not in failure_indexes:
            conn.execute(text(f"ALTER TABLE `{failure_table}` ADD KEY idx_image_name (image_name)"))
        if "idx_file_sha256" not in failure_indexes:
            conn.execute(text(f"ALTER TABLE `{failure_table}` ADD KEY idx_file_sha256 (file_sha256)"))


def _record_guarantee_parse_failure(
    conn: Any,
    failure_table: str,
    row: Dict[str, Any],
    error_message: str,
    ...
    conn.execute(
        text(
            ...
            INSERT INTO `{failure_table}`(
              source_id, parsed_row_id, image_name, file_sha256, error_message, extract_batch_id
            )
            VALUES(
              :source_id, :parsed_row_id, :image_name, :file_sha256, :error_message, :extract_batch_id
            )
...
def _clear_guarantee_parse_failure(conn: Any, failure_table: str, source_id: int) -> None:
    conn.execute(
        text(f"DELETE FROM `{failure_table}` WHERE source_id = :source_id"),
...
def parse_catfood_guarantee_values(
    ...
    info_table = _safe_table(info_table)
    guarantee_table = _safe_table(guarantee_table)
    failure_table = DEFAULT_FAILURE_TABLE
    ensure_parsed_table(engine, parsed_table)
    ensure_product_guarantee_tables(
        engine,
    ...
    for row in rows:
        ...
        try:
            ...
            with engine.begin() as conn:
                ...
                _clear_guarantee_parse_failure(
                    conn=conn,
                    failure_table=failure_table,
                    source_id=int(row["source_id"]),
                )
            ...
            with engine.begin() as conn:
                _record_guarantee_parse_failure(
                    conn=conn,
                    failure_table=failure_table,
                    row=row_data,
