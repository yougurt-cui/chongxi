...
def run_catfood_extraction_incremental(
    ...
    SELECT
      ...
      FROM (
        SELECT
          ...
          b.comment_text,
          b.source_ingest_ts,
          (CONCAT_WS(' ', IFNULL(b.title, ''), IFNULL(b.content, ''), IFNULL(b.comment_text, '')) REGEXP :cat_ctx_re) AS cat_ctx_hit,
          (CONCAT_WS(' ', IFNULL(b.title, ''), IFNULL(b.content, ''), IFNULL(b.comment_text, '')) REGEXP :brand_re) AS brand_ctx_hit,
          (CONCAT_WS(' ', IFNULL(b.title, ''), IFNULL(b.content, ''), IFNULL(b.comment_text, '')) REGEXP :health_re) AS health_ctx_hit,
    ...
    WHERE (s.brand_ctx_hit = 1 OR s.health_ctx_hit = 1)
