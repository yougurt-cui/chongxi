...
def build_sku_feature_sql(engine):
    ...
    return f"""
        ...
        LEFT JOIN {FIBER_FEATURE_TABLE} fiber
          ...
          OR (s.formula_id IS NULL AND s.sku_id = fiber.product_key)
          OR (
              COALESCE(TRIM(s.sku_name), '') <> ''
...
def load_reference_pool(engine, reference_pool_version, feature_version):
    """
    Prefer formula_id for reference pool matching; fall back to sku_id for legacy rows.
    reference_pool_version + feature_version still separates soft-stool reference pools.
    """
    sql = f"""
        SELECT
            f.formula_id,
